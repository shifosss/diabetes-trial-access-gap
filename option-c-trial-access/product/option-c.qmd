...
2. **Modeling aim (Section 8):** Determine whether including trial-access features improves cross-validated prediction of county-level diabetes burden beyond an SES-only baseline, and identify which features carry the most predictive signal.
...
This section addresses **Aim 2**: does including trial-access features improve
cross-validated prediction of county diabetes prevalence (`places_diabetes`)
beyond an SES-only baseline, and which features carry the most predictive signal?

Each model is fit in two configurations — **SES-only** (11 predictors) and
**Full** (13 predictors, adding `log_trials` and `log_dist`) — so the
incremental lift from trial-access features can be quantified directly.

| Model | Purpose |
|-------|---------|
| **Elastic Net** | Regularized linear predictor; coefficient magnitudes indicate feature importance under linearity; serves as linear baseline |
| **Random Forest** | Nonlinear ensemble; permutation importance ranks predictive features; captures interactions |
| **XGBoost** | Gradient boosting; highest predictive accuracy; SHAP decomposes predictions into per-feature contributions for importance ranking |
...
### SES-Only Baseline Models

To quantify the incremental predictive lift from trial-access features, we
refit all three models using **only the 11 SES/demographic predictors**
(dropping `log_trials` and `log_dist`) under the same CV scheme.
...
The table below compares 5-fold cross-validated performance for each model
in both configurations: **SES-only** (11 predictors) and **Full** (13 predictors,
adding trial-access features). The **ΔR²** column shows the incremental lift
from adding trial-access features.
...
# Persist the table so downstream reports (writing-report.qmd) can
# load the exact numbers without re-running the modeling pipeline.
...
The primary evidence comes from the **baseline vs. augmented model comparison**:
whether adding `log_trials` and `log_dist` to the SES-only feature set improves
cross-validated prediction accuracy (R², RMSE, MAE). All three model families
(Elastic Net, Random Forest, XGBoost) contribute equally to this comparison.

Key questions for **Aim 2**:

1. **Does adding trial-access features improve prediction of county diabetes burden beyond SES alone?**
   - Compare cross-validated R² between the SES-only model and the full
     SES + trial-access model. If the full model shows a meaningful
     improvement, trial access carries incremental predictive signal.
   - If performance is essentially unchanged, trial access is a redundant
     predictor once SES features are already in the model.
...
2. **Do non-linear effects matter?**
   ...
   - If performance is similar, linear features capture most of the predictive
     signal and the additional complexity of tree models is not justified.

3. **Feature importance ranking (SHAP)**
   - The group-level SHAP bar chart identifies which predictor groups carry
     the most predictive signal in the XGBoost model. Permutation importance
     from the Random Forest and coefficient magnitudes from the Elastic Net
     provide complementary views of feature importance under different
...
here (Aim 2) test the complementary question: whether trial-access features
add predictive value for county diabetes burden beyond what SES features
already capture, or whether the information in trial-access variables is
redundant with socioeconomic structure.

**Ecological caution:** All results reflect aggregate (county-level) patterns.
Individual-level conclusions are not warranted (ecological fallacy).
No causal claims should be drawn from these cross-sectional prediction models.
